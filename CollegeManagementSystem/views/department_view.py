...
class DepartmentView:
    ...
    def update_department(self) -> None:
        """Collect a department_id and new values, then display the result."""
        print("\n--- Update Department ---")
        try:
            department_id = int(input("Enter department ID: "))
            department_name = input("Enter new department name: ")
            hod_name = input("Enter new HOD name: ")

            message = self.controller.update_department(department_id, department_name, hod_name)
            print(message)
        except ValueError:
            print("Invalid input. Please enter a valid integer for department ID.")
